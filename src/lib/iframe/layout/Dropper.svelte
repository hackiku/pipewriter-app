...
<script lang="ts">
	...
	import DropperGrid from "../features/dropper/DropperGrid.svelte";
	import DropperBar from "../features/dropper/DropperBar.svelte";
	import ChainDropper from "../features/dropper/ChainDropper.svelte";
	...
	async function handleElementSelect(
		event: CustomEvent<{ elementId: string }>,
	) {
		...
		try {
			console.log(
				`Attempting to insert element: ${elementId} (${$elementsThemeStore})`,
			);
...
<!-- <div
	...
	${
		$elementsThemeStore === "light"
			? "bg-gray-100 dark:bg-gray-900/50"
			: "bg-gray-800/90 dark:bg-gray-100"
	}`}
> -->
	<!-- Status Bar -->
	{#if status}
