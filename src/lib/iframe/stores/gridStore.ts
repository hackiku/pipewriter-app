...
function createGridStore() {
	...
	return {
		subscribe,
		setColumns: (cols: GridColumns) => update(state => ({ ...state, columns: cols })),
		cycleColumns: () => update(state => {
...
export const gridClass = derived(
	gridStore,
	$store => ({
		grid: `grid-cols-${$store.columns}`,
		gap: $store.columns === 1 ? 'gap-3' : 'gap-2',
		padding: $store.columns === 1 ? 'px-4' : 'px-2'
	})
);