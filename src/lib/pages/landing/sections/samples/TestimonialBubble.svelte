...
<script lang="ts">
  ...
  // Position classes based on mobile/desktop
  $: positionClasses = isMobile 
    ? "bottom-6 right-6" 
    : "bottom-8 right-8";
    
  $: bubbleDirection = isMobile ? "bottom-full mb-3" : "bottom-full mb-3";
</script>
...
<div 
  bind:this={bubbleRef}
  class="absolute {positionClasses} z-20"
>
  <!-- Expanded Bubble -->
  {#if isExpanded}
    <div 
      class="absolute {bubbleDirection} right-0 w-64 p-4 bg-white dark:bg-zinc-900 
    ...
    >
      ...
      <div class="absolute top-full right-6 w-0 h-0 border-l-4 border-r-4 border-t-4 
                  border-l-transparent border-r-transparent border-t-white dark:border-t-zinc-900">
      </div>
    </div>
  {/if}
  ...
  <!-- Avatar Button -->
  <button
    class="relative w-12 h-12 rounded-full border-2 border-white dark:border-zinc-800 
