...
<div class="relative w-full p-6 rounded-xl border border-white/10 bg-card hover:shadow-md shadow-white">
  ...
  <div class="relative flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
    <div class="flex items-center gap-4">
      <div class="w-11 h-11  rounded-full bg-muted overflow-hidden">
      ...
      <div>
        ...
        <div class="text-sm text-muted-foreground">
          {testimonial.role}
          {#if testimonial.company}, {testimonial.company}
          {/if}
        </div>
      </div>
    </div>
    ...
    {#if testimonial.hire}
      <Button 
        variant="outline" 
        size="sm"
        class="absolute -bottom-6 -right-8 -rotate-[8deg] text-sm font-medium 
