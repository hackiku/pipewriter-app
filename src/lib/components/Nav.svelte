...
<nav class="fixed top-4 left-0 right-0 z-50 bg-background shadow-sm rounded-full 
  ...
  <div class="container mx-auto px-4 py-4 flex justify-between items-center">
    ...
    <div class="hidden md:flex items-center space-x-4">
      ...
      <Button on:click={toggleMode} variant="outline" size="icon" class="mr-2">
  ...
  {#if isMenuOpen}
    <div class="md:hidden px-4 py-2 border-t">
      <div class="space-y-2">
        ...
        <a 
          href="/early"
          class="block w-full px-3 py-2 text-sm font-medium text-white bg-primary hover:bg-primary/90 rounded-md text-center"
        >
          Early Access
        </a>
      </div>
